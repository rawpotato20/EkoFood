...
const ViewUser = ({ data }: { data: User }) => {
  const [open, setOpen] = useState(false);
  const [selectedData, setSelectedData] = useState<ProductInOrder[]>([]);
      ...
      <div className="container mx-auto h-[80vh] overflow-y-scroll">
        <div className="space-y-4">
          ...
          <div className="space-y-2">
            ...
            {data.cart.delivery_choice ? data.cart.delivery_choice : ""} -{" "}
            {data.cart.delivery_provider ? data.cart.delivery_provider : ""}
          </div>
          <div className="space-y-2">
          ...
          <div className="space-y-2">
            ...
            {data.box_id.current_box_status
              ? "Yes"
              : `No, last active ${new Date(data.box_id.updated_at).toLocaleDateString()}`}
          </div>
