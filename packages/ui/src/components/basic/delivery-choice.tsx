...
import track from "@/pages/api/view/track";
...
const DeliveryChoice = (props) => {
    ...
    useEffect(() => {
        if (props.settings && provider && deliveryChoice) {
            setShippingFee(
                props.settings.shipping_fees[provider][deliveryChoice]
            );
        }
    ...
    const getNearby = async () => {
        ...
        if (navigator.geolocation) {
            ...
            navigator.geolocation.getCurrentPosition(async (position) => {
                ...
                const ress = await fetch("/api/view/nearby", {
                    method: "POST",
                    headers: {
                        "Content-Type": "application/json",
                    },
    ...
    const updateCartDelivery = async (
        ...
        if (!full_name || !house_no || !street_name || !city || !postal_code || !phone_No ) {
              ...
              <div className="my-1 md:my-5 space-y-4 z-10">
                ...
                {provider && deliveryChoice && (
                  <>
                    <div>
                      Siuntimas:{" "}
                      {calculateShippingFees(provider, deliveryChoice)} €
                      <br />
