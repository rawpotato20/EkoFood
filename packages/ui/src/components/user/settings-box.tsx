...
interface SettingsBoxProps {
  data: {
    ...
    current_box_final_price?: number;
  };
  cart: {
    shipping_fee?: number;
  };
  ...
  return (
    <>
      <div className="w-full space-y-7">
        <div>
          <h1 className="text-xl md:text-2xl font-medium">Jūsų dežė:</h1>
        </div>
            ...
            <div className="flex flex-row justify-between w-1/2 md:w-1/3">
              <div>
                {props.data.current_box_price
                  ? Number(props.data.current_box_price).toFixed(2)
                  : 0}{" "}
                €
              </div>
            </div>
            ...
            <div className="flex flex-row justify-between w-1/2 md:w-1/3">
              <div>
                {props.data.current_box_final_price
                  ? Number(props.data.current_box_final_price).toFixed(2)
                  : 0}{" "}
                €
              </div>
            </div>
