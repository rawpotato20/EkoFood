...
//TODO: Toast, Nookies and React Icons
...
const SettingsAccount = (props: SettingsAccountProps) => {
  ...
  const handleUpdate2 = async (e: React.FormEvent<HTMLFormElement>) => {
    e.preventDefault();
    setLoading(true);
  ...
  const handleChoiceTranslation = (choice: string) => {
    switch (choice) {
      case "mail":
        return "paštomatas";
      case "courier":
  ...
  const showCardIcon = (card: string) => {
    switch (card) {
      case "visa":
  ...
  const handleOpen5 = (b: boolean) => {
    setOpen5(b);
  };

  const handleLogo = (s: string) => {
      ...
      <div className="w-full space-y-7">
        ...
        <div className="space-y-2 text-xs md:text-sm">
          ...
          <div className="border-b-2 border-primary flex flex-row justify-between pb-1">
            ...
            <div className="flex flex-row justify-between w-1/2 md:w-1/3">
              <div>
                {typeof props.data?.email === "string" &&
                props.data.email.includes("@") ? (
                  (() => {
              ...
              <div className="">
                {props.cart && props.cart.delivery_choice ? (
                  <span className="flex flex-row items-center">
                    {typeof props.cart.delivery_provider === "string" ? (
                      <Image
                        src={
                          handleLogo(props.cart.delivery_provider) ??
                          "/fallback-logo.png"
                        }
                ...
                <button
                  ...
                  onClick={() => {
                    if (props.data.payment_connected) {
                      ...
                      setValue(String(props.data.payment_connected));
                    } else {
                      setOpen6(true);
                      document.body.style.overflow = "hidden";
                    }
