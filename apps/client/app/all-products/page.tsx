import AllNav from "@/packages/ui/src/components/basic/all-nav";
import ProductItem from "@/packages/ui/src/components/dashboard/product-item";
...
interface Product {
  ...
  // plus any other fields the product has
}

interface User {
  cart_id: string;
  box_id: string;
  // plus any other fields the user has
}
...
const AllProducts = () => {
  ...
  const [isRestoringScroll, setIsRestoringScroll] = useState<boolean>(true); // State to control when to restore the scroll
  ...
  useEffect(() => {
    ...
    if (savedScrollPosition !== null && scrollDivRef.current !== null) {
      // Add a slight delay to ensure the div is fully rendered
      const scrollEl = scrollDivRef.current;

      setTimeout(() => {
        scrollEl.scrollTop = parseInt(savedScrollPosition, 10);
